      ...
      >
        <div className=" px-7">
          ...
          <div className="flex items-center gap-6 pb-5 border-b dark:border-darkborder mt-5 mb-3">
            ...
            />
            <div>
              <h5 className="card-title text-base font-medium dark:text-white text-dark">{session?.user?.name}</h5>
        ...
        <Button color={'primary'} as={Link} href="/auth/auth1/login" onClick={() => signOut()} className="w-full">Logout</Button>
        </div>
      </Dropdown>
    </div>
  );
